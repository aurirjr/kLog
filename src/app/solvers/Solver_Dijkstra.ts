...
export class Solver_Dijkstra
{
  ...
  public reachable_nodes_greedy(g : Graph , nK : Node, Avoid : Array<Node>) : Array<Node>
  {
  ...
  //Utilizado em ROUTE
  public find_best_route_Dijkstra(graph: Graph, A: Node , B: Node) : Path
  {
    ...
    var todos_os_nodes_acessiveis_por_A : Array<Node> = this.reachable_nodes_greedy(graph, A, avoid);
    var todos_os_nodes_acessiveis_por_B : Array<Node>  = this.reachable_nodes_greedy(graph, B, avoid); //COMENTADO
  ...
  public GUI_calcular_melhor_rota() {
    ...
    var caminho_otimo : Path = this.find_best_route_Dijkstra(new Graph()._nodes(A.a._P.p.g.nodes)._edges(A.a._P.p.g.edges), nodeA, nodeB);
    ...
    if(caminho_otimo != null) {
      ...
      //Colocando melhor rota no output:
      A.a.output_line(resultado_pro_output);

    } else {

      A.a.output_line("Não existe caminho possivel!");
